...
public class Pch01Service extends Service {
    ...
    public void detailsCheck(Pch01List list) {
        ...
        // list.setReTxt(MessageTools.getMsgText(rbms, "MST07_CHECK_RETURN_TEXT", list.getList().size(), sucC, errC));
        // list.setReTxt("aaaaa");
        int errC = 0;
        int sucC = 0;
        int count = 0;
        if (list.getList().size() == 0) {
            ...
            for (Pch01 s : list.getList()) {
                ...
                list.setReTxt(MessageTools.getMsgText(rbms, "PCH01_CHECK_RETURN_TEXT", list.getList().size(), sucC, errC));
            }
        }
    }
