...
@Repository
public class T11MailTempDao extends Dao {
    // 创建
    ...
    // Get Template
    public T11MailTemplate getTemplate(String templateId) {
        Optional<T11MailTemplate> template = db
                .run(Select.from(Sys_.T11_MAIL_TEMPLATE).where(o -> o.TEMPLATE_ID().eq(templateId)))
                .first(T11MailTemplate.class);
